...
uploaded_file = None #temporary fix
if uploaded_file is not None:
    file_to_use = uploaded_file
    st.success("Using uploaded FITS file. **Warning!** Code not currently set up to do this.")
else:
    default_path = "jw04094-o003_t003_miri_ch3-shortmediumlong_s3d.fits" #dataset that is automatically loaded
    if os.path.exists(default_path):
        file_to_use = default_path
        st.info("Using default FITS file.")
    else:
        st.error("No file uploaded and default FITS file not found.")
        st.stop()
...
#to convert to rest frame wavelength
    data_cube, data_cube_err, spectral_axis, header = util.spectral_convert(file_to_use, float(z_input), plot = True)
else:
    data_cube, data_cube_err, spectral_axis, header = util.spectral_convert(file_to_use, float(z_input), plot = False)
